...
.Example
    'c:\bravo', 'c:\charlie' | Invoke-Robocopy -Source 'c:\alpha' -ArgumentList @('/xo', '/e' ,'/np')
...
function Invoke-Robocopy
{
        ...
        function New-RobocopyHelper
        {
            ...
            if (! ([System.Management.Automation.PSTypeName]$TypeName).Type) {
                try {
                    ...
                    $ModuleBuilder = $AssemblyBuilder.DefineDynamicModule($TypeName)
    ...
    Process
    {
        foreach ($item in $Destination) {
            ...
            $AllArguments = @(
                (Resolve-Path -Path $Source).ProviderPath -replace '\\+$'
            ) + (
                $item | ForEach-Object {
            ...
            if ($Result.ExitCode -band [Robocopy.ExitCode]'FailedCopyAttempts, FatalError') {
                    ...
                    $ErrorMessage | ForEach-Object{ Write-Log -Level ERROR -Message $_ }
                ...
                {
                    Write-Log -Level INFO -Message "Robocopy from ${Source} to ${Destination} Succeeded"
                }
            }
        }
    }
